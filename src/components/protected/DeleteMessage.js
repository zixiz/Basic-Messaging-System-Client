    ...
    renderActions(){
        ...
        return (
            <React.Fragment>
                ...
                <Button as={Link} to={currentUserId === messageToDelete.sender ? "/inbox/sent" : "/inbox"} >Cancel</Button>
            </React.Fragment>
        )
    }
    ...
    renderModal(){
        ...
        if(failed_show_message){
            return(
                <CustomeModal
                icon='thumbs down'
                closeModalPath = "/inbox"
                title={messageToDelete.error}
                content=''
                actions={<Button as={Link} to={"/inbox"} >Cancel</Button>}
                />
                )
        }else{
            return(
                <CustomeModal
                icon='archive'
                closeModalPath = {currentUserId === messageToDelete.sender ? "/inbox/sent" : "/inbox"}
